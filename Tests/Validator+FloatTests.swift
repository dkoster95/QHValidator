...
class Validator_FloatTests: XCTestCase {

    let subject = Validator<Float>()
    func testBetween() {
    ...
    func testGreaterEqualsTrue() {
        let result = try! subject.greaterOrEqualsThan(value: 123.5).validate(value: 123.5)
        XCTAssertTrue(result)
    }
